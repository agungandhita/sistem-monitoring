...
<div class="bg-white rounded-lg shadow-md p-6 mt-24">
    <div class="flex justify-between items-center mb-6">
        <div>
            ...
            <p class="text-gray-600">Kelola data kelas dan tingkatan</p>
        </div>
        ...
        <table class="min-w-full bg-white border border-gray-200 rounded-lg">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Jumlah Rombel</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200" id="kelasTableBody">
                ...
                <tr class="hover:bg-gray-50 transition duration-200" 
                            ...
                            <div class="flex-shrink-0 h-10 w-10">
                                <div class="h-10 w-10 rounded-full bg-blue-100 flex items-center justify-center">
                                    <span class="text-blue-600 font-medium text-sm">
                                        {{ $item->tingkat }}{{ substr($item->nama_kelas, 0, 1) }}
                                    </span>
                                </div>
                            </div>
                            <div class="ml-4">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        {{ $item->kapasitas_maksimal ?? 'Tidak terbatas' }}
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                            {{ $item->rombels->count() }} Rombel
                        </span>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <div class="flex space-x-2">
                            ...
                            <button onclick="confirmDelete({{ $item->kelas_id }})" 
                ...
                <tr>
                    <td colspan="8" class="px-6 py-4 text-center text-gray-500">
                        <div class="flex flex-col items-center justify-center py-8">
                            ...
                            <p class="text-lg font-medium text-gray-900 mb-2">Belum ada data kelas</p>
                            <p class="text-gray-500 mb-4">Mulai dengan menambahkan kelas baru</p>
                            <a href="{{ route('admin.kelas.create') }}" class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-lg transition duration-200">
...
<script>
    ...
    function filterTable() {
        ...
        const tingkatValue = document.getElementById('tingkatFilter').value;
        ...
        rows.forEach(row => {
            ...
            const matchesTingkat = !tingkatValue || tingkat === tingkatValue;
            const matchesTahun = !tahunValue || tahun === tahunValue;
            const matchesStatus = !statusValue || status === statusValue;

            if (matchesSearch && matchesTingkat && matchesTahun && matchesStatus) {
                row.style.display = '';
            } else {
    ...
    function confirmDelete(kelasId) {
        document.getElementById('deleteForm').action = `{{ url('admin/kelas') }}/${kelasId}`;
