...
<div class="p-6">
    ...
    <div class="mb-8">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-bold text-gray-900">Nilai Harian - {{ $siswa->nama }}</h1>
                <p class="text-gray-600 mt-2">NIS: {{ $siswa->nis }} | Kelas: {{ $siswa->kelas->nama_kelas ?? 'N/A' }}</p>
            </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
        <h3 class="text-lg font-semibold text-gray-900 mb-4">Filter Nilai</h3>
        <form method="GET" action="{{ route('wali.nilai-harian.show', $siswa->siswa_id) }}" class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <!-- Mata Pelajaran -->
            <div>
                ...
                <select name="mapel_id" id="mapel_id" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    ...
                    @foreach($mapels as $mapel)
                        <option value="{{ $mapel->mapel_id }}" {{ $mapelId == $mapel->mapel_id ? 'selected' : '' }}>
                            {{ $mapel->mapel }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-bold text-gray-900">{{ $statistics['total_nilai'] }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-bold text-gray-900">{{ $statistics['rata_rata'] }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-bold text-gray-900">{{ $statistics['nilai_tertinggi'] }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-bold text-gray-900">{{ $statistics['nilai_terendah'] }}</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    @if($nilaiByMapel->count() > 0)
        <!-- Nilai by Subject -->
        @foreach($nilaiByMapel as $namaMapel => $nilaiList)
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
                <h3 class="text-lg font-semibold text-gray-900 mb-4">{{ $namaMapel }}</h3>
                ...
                <div class="mt-4 p-4 bg-gray-50 rounded-lg">
                    <p class="text-sm text-gray-600">
                        <span class="font-medium">Rata-rata {{ $namaMapel }}:</span>
                        <span class="text-lg font-bold ml-2
                            @if($nilaiList->avg('nilai') >= 80) text-green-600
                            @elseif($nilaiList->avg('nilai') >= 70) text-yellow-600
                            @else text-red-600 @endif">
                            {{ round($nilaiList->avg('nilai'), 2) }}
                        </span>
                    </p>
                </div>
            </div>
    ...
    @else
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-12 text-center">
            ...
            <p class="text-gray-500">Belum ada nilai harian untuk periode yang dipilih.</p>
        </div>
    @endif
</div>
@endsection